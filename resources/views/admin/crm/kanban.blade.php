...
@section('title', 'Lead Pipeline — Kanban Board')
...
<div class="kb-wrapper">
    <div class="kb-board">
        ...
        <div class="kb-col" data-status="{{ $status }}" style="--col-color:{{ $col['color'] }};">
            ...
            <div class="kb-col-head">
                ...
                <div class="d-flex align-items-center gap-2">
                    <span class="kb-col-badge">{{ $columns[$status]->count() }}</span>
                    <a href="{{ route('admin.crm.leads', ['status'=>$status]) }}" class="kb-col-all">all&nbsp;→</a>
                </div>
            </div>

            {{-- Cards --}}
            <div class="kb-cards" data-status="{{ $status }}">
                ...
                @endphp
                <div class="kb-card {{ $lead->isOverdue() ? 'kb-card--overdue' : '' }}"
                    ...
                    {{-- Top row: avatar + name + ref --}}
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <div class="kb-avatar" style="--col-color:{{ $col['color'] }};">{{ $initials }}</div>
                        <div style="flex:1;min-width:0;">
                            <div class="fw-bold text-truncate" style="font-size:.83rem;color:#1a1f2e;line-height:1.2;">{{ $lead->name }}</div>
                            <div class="kb-ref">{{ $lead->reference }}</div>
                        </div>
                        {{-- Urgent badge --}}
                        @if($lead->isOverdue())
                        ...
                        @elseif($status === 'tour_scheduled')
                            ...
                            @elseif($daysUntil > 0 && $daysUntil <= 2)
                                <span class="kb-pill kb-pill--soon">In {{ $daysUntil }}d</span>
                            @endif
                        @endif
                    </div>
                    ...
                    <div style="font-size:.74rem;color:#6c757d;margin-bottom:8px;">
                        ...
                        <span style="color:#374151;">{{ $lead->child_name }}</span>
                        <span class="ms-1 kb-pill" style="background:#f3f4f6;color:#6c757d;padding:1px 7px;">{{ $lead->child_age }}</span>
                    </div>

                    <div class="kb-card-divider"></div>

                    {{-- Bottom row: date + source + app indicator + assignee --}}
                    <div class="d-flex align-items-center justify-content-between" style="gap:6px;">
                        <span style="font-size:.7rem;color:#adb5bd;">
                            <i class="fas fa-calendar-alt me-1"></i>{{ $lead->preferred_date->format('d M') }}
                        </span>
                        <div class="d-flex align-items-center gap-1 flex-wrap justify-content-end">
                            @if($lead->relationLoaded('application') && $lead->application)
                                <span class="kb-pill kb-pill--app"><i class="fas fa-file-alt me-1"></i>App</span>
                            @endif
                            @if($lead->source)
                                <span class="kb-src {{ $srcClass[$lead->source] ?? 'kb-src--other' }}">{{ ucfirst($lead->source) }}</span>
                            @endif
                            @if($lead->assignee)
                                <span style="font-size:.68rem;color:#adb5bd;" title="{{ $lead->assignee->name }}">
                                    <i class="fas fa-user-circle me-1"></i>{{ explode(' ', $lead->assignee->name)[0] }}
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                @empty
                <div class="kb-empty">
                    <i class="{{ $col['icon'] }}" style="color:{{ $col['color'] }};opacity:.25;"></i>
                    No leads here
                </div>
                @endforelse
            </div>
...
<div class="kb-hidden-strip">
    ...
    <a href="{{ route('admin.crm.leads', ['status'=>'waitlist']) }}" class="kb-hidden-pill kb-hidden-pill--waitlist">
        <i class="fas fa-clock"></i>Waitlist
        <span style="opacity:.6;">({{ $hiddenCounts['waitlist'] }})</span>
    </a>
    <a href="{{ route('admin.crm.leads', ['status'=>'lost']) }}" class="kb-hidden-pill kb-hidden-pill--lost">
        <i class="fas fa-times-circle"></i>Lost
        <span style="opacity:.6;">({{ $hiddenCounts['lost'] }})</span>
    </a>
</div>
