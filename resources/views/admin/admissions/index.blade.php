...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('admin.admissions.index', array_merge(request()->query(), ['export' => 'csv'])) }}"
    ...
    <div class="adm-table-wrap">
        <table class="table adm-table mb-0">
            ...
            <tbody>
                ...
                @endphp
                <tr class="{{ $isUrgent ? 'adm-row-urgent' : '' }}">
                    {{-- Reference --}}
                    <td>
                        <code style="font-size:.76rem;color:#0077B6;font-weight:700;letter-spacing:.3px;">{{ $app->reference }}</code>
                        <div style="font-size:.7rem;color:#adb5bd;margin-top:2px;">
                            <i class="fas fa-calendar-alt me-1"></i>{{ $app->created_at->format('d M Y') }}
                        </div>
                        @if($isUrgent)
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div>
                                <div class="fw-semibold" style="color:#1a1f2e;line-height:1.2;">{{ $app->child_name }}</div>
                    ...
                    {{-- Parent --}}
                    <td>
                        <div style="color:#1a1f2e;font-weight:600;font-size:.84rem;">{{ $app->mother_name }}</div>
                        <a href="mailto:{{ $app->mother_email }}" style="font-size:.74rem;color:#0077B6;text-decoration:none;display:block;margin-top:1px;">
                            <i class="fas fa-envelope me-1" style="font-size:.6rem;color:#adb5bd;"></i>{{ $app->mother_email }}
                        </a>
                        <a href="tel:{{ $app->mother_cell }}" style="font-size:.74rem;color:#adb5bd;text-decoration:none;">
                            <i class="fas fa-phone me-1" style="font-size:.6rem;"></i>{{ $app->mother_cell }}
                        </a>
                    </td>

                    {{-- Programme --}}
                    <td>
                        <span class="adm-prog" style="background:{{ $pBg }};color:{{ $pClr }};">{{ $app->program_name }}</span>
                        <div style="font-size:.72rem;color:#adb5bd;margin-top:4px;">
                            <i class="fas fa-tag me-1" style="font-size:.6rem;"></i>{{ ucfirst($app->fee_option) }}
                        </div>
                    </td>

                    {{-- Start Date --}}
                    <td>
                        <div style="color:#1a1f2e;font-weight:600;font-size:.83rem;">{{ $app->start_date->format('d M Y') }}</div>
                        <div style="font-size:.72rem;color:#adb5bd;margin-top:1px;">{{ $app->start_date->format('l') }}</div>
                    </td>
                    ...
                    <td>
                        @if($app->lead)
                            ...
                            <a href="{{ route('admin.crm.leads.show', $app->lead) }}"
                               title="View Lead — {{ $app->lead->reference }}" style="text-decoration:none;">
                                <span class="adm-src" style="background:{{ $sBg }};color:{{ $sClr }};">
                                    {{ ucfirst($app->lead->source ?? 'N/A') }}
                                </span>
                            </a>
                            <div style="font-size:.7rem;color:#adb5bd;margin-top:3px;">
                                <i class="fas fa-link me-1"></i>
                                <a href="{{ route('admin.crm.leads.show', $app->lead) }}"
                                   style="color:#adb5bd;text-decoration:none;">{{ $app->lead->reference }}</a>
                            </div>
                        @else
                    ...
                    {{-- Status --}}
                    <td>
                        <span class="adm-pill" style="background:{{ $stBg }};color:{{ $stClr }};">
                            {{ $app->statusLabel() }}
                        </span>
                        @if($app->invited_at)
                            <div style="font-size:.68rem;color:#0077B6;margin-top:4px;font-weight:600;">
                                <i class="fas fa-paper-plane me-1"></i>Invited
                                <span style="color:#adb5bd;font-weight:400;">{{ $app->invited_at->format('d M') }}</span>
                            </div>
                        @endif
                        @if($app->approved_at && $app->status === 'approved')
                            <div style="font-size:.68rem;color:#adb5bd;margin-top:3px;">
                                <i class="fas fa-check me-1"></i>{{ $app->approved_at->format('d M') }}
                            </div>
                        @endif
                    </td>
                    ...
                    <td colspan="8" class="text-center" style="padding:60px 20px;">
                        ...
                        <div style="font-size:.82rem;color:#adb5bd;">
                            @if(request()->hasAny(['status','program','search']))
                                ...
                                <a href="{{ route('admin.admissions.index') }}" style="color:#7c3aed;text-decoration:none;font-weight:600;">Clear filters</a>
                            @else
