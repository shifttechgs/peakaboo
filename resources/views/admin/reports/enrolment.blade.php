...
<div class="rpt-stat-grid">
    ...
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#16a34a;">{{ $approved }}</div>
        <div class="rpt-stat-lbl">Approved</div>
        <div class="rpt-stat-sub">{{ $total > 0 ? round($approved / $total * 100) : 0 }}% approval rate</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#d97706;">{{ $pending }}</div>
        <div class="rpt-stat-lbl">Actionable</div>
        <div class="rpt-stat-sub">Pending + under review</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#6c757d;">{{ $waitlist }}</div>
        <div class="rpt-stat-lbl">Waitlist</div>
        <div class="rpt-stat-sub">Awaiting capacity</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#ef4444;">{{ $rejected }}</div>
        <div class="rpt-stat-lbl">Rejected</div>
        <div class="rpt-stat-sub">{{ $total > 0 ? round($rejected / $total * 100) : 0 }}% rejection rate</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val">{{ $thisMonth }}</div>
        <div class="rpt-stat-lbl">This Month</div>
        <div class="rpt-stat-sub" style="color:{{ $monthDiff >= 0 ? '#16a34a' : '#ef4444' }};">
            {{ $monthDiff >= 0 ? '+' : '' }}{{ $monthDiff }} vs last month
        </div>
    </div>
    @if($avgDaysToApproval)
    <div class="rpt-stat">
        <div class="rpt-stat-val">{{ round($avgDaysToApproval) }}</div>
...
<div class="col-lg-8">
    ...
    <div class="rpt-panel">
        ...
        <div class="rpt-panel-body">
            @php $maxBar = max(1, $trend->max()); @endphp
            <div class="bar-chart">
                @foreach($trend as $month => $count)
                @php $pct = round(($count / $maxBar) * 100); @endphp
                <div class="bar-wrap" title="{{ \Carbon\Carbon::createFromFormat('Y-m', $month)->format('M Y') }}: {{ $count }}">
                    @if($count > 0)<div class="bar-val">{{ $count }}</div>@endif
                    <div class="bar" style="height:{{ max(4, $pct) }}%;background:{{ $month === now()->format('Y-m') ? '#7c3aed' : '#e9d5ff' }};"></div>
    ...
    <div class="rpt-panel">
        <div class="rpt-panel-header">
            ...
            <a href="{{ route('admin.admissions.index') }}" style="font-size:.76rem;color:#0077B6;text-decoration:none;">View all</a>
        </div>
        <div style="overflow-x:auto;">
            <table class="rpt-table">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td><a href="{{ route('admin.admissions.show', $app->id) }}" style="color:#0077B6;text-decoration:none;font-weight:600;font-size:.78rem;"><code>{{ $app->reference }}</code></a></td>
                        <td style="font-weight:600;color:#1a1f2e;">{{ $app->child_name }}</td>
                        <td style="color:#6c757d;">{{ $app->program_name ?? '—' }}</td>
                        <td><span class="rpt-pill" style="background:{{ $stBg }};color:{{ $stC }};">{{ $app->statusLabel() }}</span></td>
                        <td style="color:#94a3b8;">{{ $app->created_at->format('d M Y') }}</td>
                    </tr>
                    @empty
...
<div class="col-lg-4">
    ...
    <div class="rpt-panel">
        ...
        <div class="rpt-panel-body">
            ...
            @endphp
            @foreach($statuses as $label => [$count, $colour])
            <div class="prog-row">
                <div class="prog-label">{{ $label }}</div>
                <div class="prog-bar-wrap">
                    <div class="prog-bar" style="width:{{ $total > 0 ? round($count/$total*100) : 0 }}%;background:{{ $colour }};"></div>
                </div>
    ...
    <div class="rpt-panel">
        ...
        <div class="rpt-panel-body">
            ...
            @endphp
            @forelse($byProgram as $prog => $count)
            <div class="prog-row">
                <div class="prog-label" style="font-size:.78rem;">{{ $prog }}</div>
                <div class="prog-bar-wrap">
                    <div class="prog-bar" style="width:{{ round($count/$progMax*100) }}%;background:{{ $progColors[$prog] ?? '#7c3aed' }};"></div>
                </div>
                <div class="prog-count">{{ $count }}</div>
            </div>
            @empty
    ...
    <div class="rpt-panel">
        ...
        <div class="rpt-panel-body">
            ...
            <div class="prog-row">
                <div class="prog-label" style="font-size:.78rem;">{{ ucfirst($src) }}</div>
                <div class="prog-bar-wrap">
                    <div class="prog-bar" style="width:{{ round($count/$srcMax*100) }}%;background:{{ $srcColors[$src] ?? '#6c757d' }};"></div>
                </div>
                <div class="prog-count">{{ $count }}</div>
            </div>
            @endforeach
