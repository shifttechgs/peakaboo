...
<div class="d-flex justify-content-between align-items-center leads-header">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('admin.crm.leads.export', request()->query()) }}"
...
<div class="leads-panel">
    <div class="leads-panel-header">
        ...
        @if(request()->hasAny(['status','source','child_age','search','assigned_to']))
            ...
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <select name="assigned_to" class="form-select">
                        ...
                        @foreach($adminUsers as $uid => $uname)
                            <option value="{{ $uid }}" {{ request('assigned_to') == $uid ? 'selected' : '' }}>
                                {{ $uname }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" name="search" class="form-control"
                           placeholder="Name, email, ref…" value="{{ request('search') }}">
                </div>
                <div class="col-md-2">
...
<div class="bulk-bar d-none" id="bulkBar">
    ...
    <form method="POST" action="{{ route('admin.crm.leads.bulk-action') }}" id="bulkForm"
        ...
        <select name="action" class="form-select" style="max-width:200px;" required>
            ...
            <optgroup label="Change Status">
                @foreach(\App\Models\Lead::STATUSES as $s)
                    <option value="{{ $s }}">Set: {{ ucwords(str_replace('_', ' ', $s)) }}</option>
                @endforeach
            </optgroup>
    ...
    <div class="table-responsive">
        <table class="table leads-table mb-0">
            ...
            <tbody>
                ...
                @endphp
                <tr class="{{ $lead->isOverdue() ? 'overdue-row' : '' }}">
                    <td>
                        <input type="checkbox" class="form-check-input lead-checkbox" value="{{ $lead->id }}">
                    </td>
                    <td>
                        <code style="font-size:.76rem;color:#0077B6;">{{ $lead->reference }}</code>
                        @if($lead->isOverdue())
                    ...
                    <td>
                        ...
                        <a href="mailto:{{ $lead->email }}" style="font-size:.74rem;color:#adb5bd;text-decoration:none;">
                            {{ $lead->email }}
                        </a>
                        <div style="font-size:.74rem;color:#adb5bd;">{{ $lead->phone }}</div>
                    </td>
                    <td>
                        <div style="color:#374151;">{{ $lead->child_name }}</div>
                        <span class="age-pill mt-1 d-inline-block">{{ $lead->child_age }}</span>
                    </td>
                    <td>
                        <div style="color:#374151;font-weight:600;">{{ $lead->preferred_date->format('d M Y') }}</div>
                        <div style="font-size:.74rem;color:#adb5bd;">{{ $lead->preferred_time }}</div>
                    </td>
                    <td>
                        @if($lead->source)
                    ...
                    <td>
                        @if($lead->assignee)
                            <span style="font-size:.78rem;background:#f3f4f6;color:#374151;border-radius:20px;padding:3px 10px;font-weight:600;">
                                <i class="fas fa-user me-1" style="color:#94a3b8;font-size:.65rem;"></i>{{ $lead->assignee->name }}
                            </span>
                        @else
                    ...
                    <td>
                        ...
                        @else
                            ...
                            <span style="font-size:.78rem;{{ $lead->isOverdue() ? 'color:#ef4444;font-weight:700;' : 'color:#adb5bd;' }}">
