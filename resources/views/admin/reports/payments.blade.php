...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        ...
        <p style="font-size:.84rem;color:#94a3b8;margin:0;">Revenue overview, outstanding fees and payment trends &mdash; {{ now()->format('F Y') }}</p>
    </div>
...
<div class="rpt-stat-grid">
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#16a34a;">R {{ number_format($verifiedThisMonth, 2) }}</div>
        <div class="rpt-stat-lbl">Collected This Month</div>
        <div class="rpt-stat-sub">Verified payments only</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:{{ $outstandingThisMonth > 0 ? '#ef4444' : '#16a34a' }};">R {{ number_format($outstandingThisMonth, 2) }}</div>
        <div class="rpt-stat-lbl">Outstanding This Month</div>
        <div class="rpt-stat-sub">Of R {{ number_format($monthlyExpected, 2) }} expected</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val">R {{ number_format($verifiedAllTime, 2) }}</div>
        <div class="rpt-stat-lbl">Total Collected</div>
        <div class="rpt-stat-sub">All time, verified</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:{{ $pendingCount > 0 ? '#d97706' : '#16a34a' }};">{{ $pendingCount }}</div>
        <div class="rpt-stat-lbl">Pending POPs</div>
        <div class="rpt-stat-sub">Awaiting verification</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#6c757d;">{{ $outstandingParents->count() }}</div>
        <div class="rpt-stat-lbl">Parents Not Paid</div>
        <div class="rpt-stat-sub">{{ now()->format('F Y') }}</div>
    </div>
    <div class="rpt-stat">
        <div class="rpt-stat-val" style="color:#ef4444;">{{ $rejectedCount }}</div>
...
<div class="col-lg-8">
    ...
    <div class="rpt-panel">
        ...
        <div class="rpt-panel-body">
            @php $maxRev = max(1, $revenueTrend->max()); @endphp
            <div class="bar-chart">
                @foreach($revenueTrend as $month => $amount)
                @php $pct = round(($amount / $maxRev) * 100); @endphp
                <div class="bar-wrap" title="{{ \Carbon\Carbon::createFromFormat('Y-m', $month)->format('M Y') }}: R{{ number_format($amount) }}">
                    @if($amount > 0)<div class="bar-val">R{{ number_format($amount / 1000, 1) }}k</div>@endif
                    <div class="bar" style="height:{{ max(4, $pct) }}%;background:{{ $month === $currentMonth ? '#16a34a' : '#86efac' }};"></div>
    ...
    <div class="rpt-panel">
        <div class="rpt-panel-header">
            ...
            <a href="{{ route('admin.payments.statements') }}" style="font-size:.76rem;color:#0077B6;text-decoration:none;">View all</a>
        </div>
        <div style="overflow-x:auto;">
            <table class="rpt-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="color:#94a3b8;">{{ $pmt->payment_date->format('d M Y') }}</td>
                        <td style="font-weight:600;color:#1a1f2e;">{{ $pmt->parentUser?->name ?? '—' }}</td>
                        <td><code style="font-size:.75rem;color:#0077B6;">{{ $pmt->reference }}</code></td>
                        <td style="color:#6c757d;">{{ \Carbon\Carbon::createFromFormat('Y-m', $pmt->month_year)->format('M Y') }}</td>
                        <td style="text-align:right;font-weight:700;color:#16a34a;">R {{ number_format($pmt->amount, 2) }}</td>
                    </tr>
                    @empty
...
<div class="col-lg-4">
    ...
    <div class="rpt-panel">
        <div class="rpt-panel-header">
            <h6><i class="fas fa-calendar-check me-2" style="color:#16a34a;"></i>{{ now()->format('F Y') }} Summary</h6>
        </div>
        <div class="rpt-panel-body">
            @php
            ...
            <div style="margin-bottom:16px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                    ...
                    <span style="font-size:.78rem;font-weight:800;color:{{ $collectedPct >= 80 ? '#16a34a' : ($collectedPct >= 50 ? '#d97706' : '#ef4444') }};">{{ $collectedPct }}%</span>
                </div>
                <div style="height:10px;background:#f3f4f6;border-radius:5px;overflow:hidden;">
                    <div style="height:100%;width:{{ $collectedPct }}%;background:{{ $collectedPct >= 80 ? '#16a34a' : ($collectedPct >= 50 ? '#d97706' : '#ef4444') }};border-radius:5px;transition:width .3s;"></div>
                </div>
                <div style="display:flex;justify-content:space-between;margin-top:6px;font-size:.72rem;color:#94a3b8;">
                    <span>R {{ number_format($verifiedThisMonth, 2) }} collected</span>
                    <span>R {{ number_format($monthlyExpected, 2) }} expected</span>
                </div>
            </div>
            <div style="background:#f8faff;border-radius:10px;padding:14px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                    ...
                    <span style="font-size:.8rem;font-weight:700;color:#1a1f2e;">R {{ number_format($monthlyExpected, 2) }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                    <span style="font-size:.78rem;color:#6c757d;">Collected</span>
                    <span style="font-size:.8rem;font-weight:700;color:#16a34a;">R {{ number_format($verifiedThisMonth, 2) }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid #e5e7eb;">
                    <span style="font-size:.78rem;color:#6c757d;">Outstanding</span>
                    <span style="font-size:.8rem;font-weight:800;color:{{ $outstandingThisMonth > 0 ? '#ef4444' : '#16a34a' }};">R {{ number_format($outstandingThisMonth, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="rpt-panel">
        <div class="rpt-panel-header">
            <h6><i class="fas fa-exclamation-circle me-2" style="color:#ef4444;"></i>Outstanding — {{ now()->format('M Y') }}</h6>
            <span style="font-size:.72rem;font-weight:700;color:#ef4444;">{{ $outstandingParents->count() }} parent{{ $outstandingParents->count() === 1 ? '' : 's' }}</span>
        </div>
        <div class="rpt-panel-body" style="padding:12px 22px;">
            @forelse($outstandingParents as $app)
            <div class="outstanding-row">
                <div>
                    <div style="font-size:.82rem;font-weight:600;color:#1a1f2e;">{{ $app->parentUser?->name ?? $app->mother_name }}</div>
                    <div style="font-size:.72rem;color:#94a3b8;">{{ $app->child_name }}</div>
                </div>
                @if($app->parentUser)
                <a href="mailto:{{ $app->parentUser->email }}"
        ...
        @if($outstandingParents->count() > 0)
        <div style="padding:0 22px 16px;">
            <a href="{{ route('admin.payments.outstanding') }}"
