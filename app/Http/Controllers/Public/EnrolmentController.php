...
class EnrolmentController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        $year      = date('Y');
        $nextNum   = Application::whereYear('created_at', $year)->count() + 1;
        $applicationId = 'APP-' . $year . '-' . str_pad($nextNum, 3, '0', STR_PAD_LEFT);

        // Handle file uploads — store relative paths for portability
        $documentPaths = [];
        $emailDocuments = [];
        ...
        foreach ($uploadedFields as $field) {
            if ($request->hasFile($field)) {
                ...
                $fileName = $field . '_' . $applicationId . '.' . $file->getClientOriginalExtension();
                $relPath  = $file->storeAs('enrolments/' . $applicationId, $fileName, 'public');
        ...
        // Generate PDF
        $pdf         = Pdf::loadView('pdf.enrolment-application', ['data' => $validated, 'applicationId' => $applicationId]);
        $pdfFileName = 'Application-' . $applicationId . '.pdf';
        $pdfDir      = storage_path('app/public/enrolments/' . $applicationId);
        $pdfPath     = $pdfDir . '/' . $pdfFileName;
        $pdfRelPath  = 'enrolments/' . $applicationId . '/' . $pdfFileName;
        ...
        // Auto-send acknowledgement to parent
        try {
        ...
        // Send email to admin with PDF + docs attached
        $emailSent  = false;
        $emailError = null;

        try {
            Mail::to(config('mail.contact_email'))
                ->send(new EnrolmentApplicationMail($validated, $applicationId, $pdfPath, $emailDocuments));
