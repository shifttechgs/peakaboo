...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-val <?php echo e($stats['pending_count'] > 0 ? 'warn' : 'good'); ?>"><?php echo e($stats['pending_count']); ?></div>
            <div class="stat-lbl">Pending Review</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-val good"><?php echo e($stats['verified_month']); ?></div>
            <div class="stat-lbl">Verified This Month</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-val">R <?php echo e(number_format($stats['collected_month'], 0)); ?></div>
            <div class="stat-lbl">Collected This Month</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="stat-card">
            <div class="stat-val <?php echo e($stats['total_outstanding'] > 0 ? 'bad' : 'good'); ?>">R <?php echo e(number_format($stats['total_outstanding'], 0)); ?></div>
...
<div class="panel">
    <div class="panel-header">
        ...
        <div style="display:flex;gap:8px;">
            <a href="<?php echo e(route('admin.payments.outstanding')); ?>" class="btn btn-sm rounded-pill px-3" style="background:#f0f9ff;color:#0077B6;border:1.5px solid #bae6fd;font-size:.78rem;font-weight:600;">
                <i class="fas fa-exclamation-circle me-1"></i> Outstanding
            </a>
            <a href="<?php echo e(route('admin.payments.statements')); ?>" class="btn btn-sm rounded-pill px-3" style="background:#f8faff;color:#64748b;border:1.5px solid #e2e8f0;font-size:.78rem;font-weight:600;">
    ...
    <table class="table pop-table mb-0">
        ...
        <tbody>
            <?php $__currentLoopData = $pending; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pmt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr>
                <td>
                    <div style="font-weight:700;color:#1a1f2e;"><?php echo e($pmt->parentUser?->name ?? '—'); ?></div>
                    <div style="font-size:.73rem;color:#94a3b8;"><?php echo e($pmt->parentUser?->email); ?></div>
                </td>
                <td>
                    <?php if($pmt->child): ?>
                        <div style="font-weight:600;"><?php echo e($pmt->child->name); ?></div>
                        <code style="font-size:.72rem;color:#0077B6;"><?php echo e($pmt->child->child_number); ?></code>
                    <?php elseif($pmt->application): ?>
                        <div style="font-weight:600;"><?php echo e($pmt->application->child_name); ?></div>
                        <code style="font-size:.72rem;color:#94a3b8;"><?php echo e($pmt->application->reference); ?></code>
                    <?php else: ?>
                        <span style="color:#94a3b8;">—</span>
                    <?php endif; ?>
                </td>
                <td><strong style="color:#1a1f2e;">R <?php echo e(number_format($pmt->amount, 2)); ?></strong></td>
                <td style="color:#64748b;"><?php echo e($pmt->payment_date->format('d M Y')); ?></td>
                <td><code style="font-size:.78rem;color:#374151;"><?php echo e($pmt->reference); ?></code></td>
                <td style="color:#94a3b8;font-size:.78rem;"><?php echo e($pmt->created_at->format('d M Y H:i')); ?></td>
                <td>
                    <?php if($pmt->pop_path): ?>
                ...
                <td class="text-center">
                    <div style="display:flex;gap:6px;justify-content:center;">
                        ...
                        <button type="button" class="action-btn btn-verify"
                                ...
                                data-payment-id="<?php echo e($pmt->id); ?>"
                                data-child="<?php echo e($pmt->child?->name ?? $pmt->application?->child_name ?? ''); ?>"
                                data-amount="<?php echo e(number_format($pmt->amount, 2)); ?>"
                                data-ref="<?php echo e($pmt->reference); ?>">
...
<div class="modal fade" id="recordPaymentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:1px solid #f0f0f0;">
            ...
            <form method="POST" action="<?php echo e(route('admin.payments.record')); ?>">
                <?php echo csrf_field(); ?>
                <div class="modal-body" style="padding:22px 24px;">
                    <div class="mb-3">
                        ...
                        <select name="application_id" class="form-select form-select-sm" required style="border-color:#e0eeff;">
                            ...
                            <?php $__currentLoopData = \App\Models\Application::with('child')->where('status','approved')->orderBy('child_name')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $app): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($app->id); ?>">
                                <?php echo e($app->child_name); ?>

                                <?php if($app->child): ?> (<?php echo e($app->child->child_number); ?>) <?php endif; ?>
                            </option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                    <div class="row g-2">
                        ...
                        <div class="col-6">
                            ...
                            <input type="date" name="payment_date" class="form-control form-control-sm" value="<?php echo e(date('Y-m-d')); ?>" required style="border-color:#e0eeff;">
                        </div>
                    </div>
                    <div class="mt-2">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64_3.3.4\www\projects\peekaboo\resources\views/admin/payments/index.blade.php ENDPATH**/ ?>